...
let extractCommonNameUsingOpenssl = (() => {
    var _ref4 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (password, certPath) {
        const result = yield (0, (_builderUtil || _load_builderUtil()).exec)("openssl", ["pkcs12", "-nokeys", "-nodes", "-passin", `pass:${password}`, "-nomacver", "-clcerts", "-in", certPath], { timeout: 30 * 1000, maxBuffer: 2 * 1024 * 1024 }, debugOpenssl.enabled);
...
class WinPackager extends (_platformPackager || _load_platformPackager()).PlatformPackager {
    constructor(info) {
        ...
        this.computedPublisherSubjectOnWindowsOnly = new (_lazyVal || _load_lazyVal()).Lazy((0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            return yield (0, (_builderUtil || _load_builderUtil()).exec)("powershell.exe", args, { timeout: 30 * 1000 }).then(function (it) {
                return it.trim();
            });
        }));
    ...
    createTargets(targets, mapper) {
        ...
        const getHelper = () => {
            if (helper == null) {
                helper = new (_nsisUtil || _load_nsisUtil()).AppPackageHelper();
            }
            return helper;
        };
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (cscInfo == null) {
                if (_this3.platformSpecificBuildOptions.sign != null) {
                    yield (0, (_windowsCodeSign || _load_windowsCodeSign()).sign)(signOptions);
            ...
            if (certFile == null) {
                if (cscInfo.subjectName == null) {
                    (0, (_builderUtil || _load_builderUtil()).log)(`${logMessagePrefix} (certificate SHA1: "${cscInfo.certificateSha1}")`);
                } else {
                    (0, (_builderUtil || _load_builderUtil()).log)(`${logMessagePrefix} (certificate subject name: "${cscInfo.subjectName}")`);
                }
            } else {
                (0, (_builderUtil || _load_builderUtil()).log)(`${logMessagePrefix} (certificate file: "${certFile}")`);
            }
            yield WinPackager.doSign(Object.assign({}, signOptions, { cert: certFile, password: cscInfo.password, options: Object.assign({}, _this3.platformSpecificBuildOptions, { certificateSubjectName: cscInfo.subjectName, certificateSha1: cscInfo.certificateSha1 }) }));
        })();
    }
    static doSign(options) {
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            for (let i = 0; i < 3; i++) {
                try {
