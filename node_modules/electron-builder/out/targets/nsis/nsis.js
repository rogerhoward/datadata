...
// noinspection SpellCheckingInspection
const NSIS_PATH = new (_lazyVal || _load_lazyVal()).Lazy(() => (0, (_binDownload || _load_binDownload()).getBinFromGithub)("nsis", "3.0.1.13", "2921dd404ce9b69679088a6f1409a56dd360da2077fe1019573c0712c9edf057"));
// noinspection SpellCheckingInspection
...
class NsisTarget extends (_core || _load_core()).Target {
    ...
    buildAppPackage(appOutDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (isPackElevateHelper !== false) {
                yield (0, (_fs || _load_fs()).copyFile)(_path.join((yield NSIS_PATH.value), "elevate.exe"), _path.join(appOutDir, "resources", "elevate.exe"), false);
            }
            const isDifferentialPackage = options.differentialPackage;
            const format = !isDifferentialPackage && options.useZip ? "zip" : "7z";
            const archiveFile = _path.join(_this2.outDir, `${packager.appInfo.name}-${packager.appInfo.version}-${(_builderUtil || _load_builderUtil()).Arch[arch]}.nsis.${format}`);
            ...
            if (options.differentialPackage) {
                ...
                return yield (0, (_appPackageBuilder || _load_appPackageBuilder()).createPackageFileInfo)(archiveFile);
            }
        })();
    }
    ...
    finishBuild() {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            (0, (_builderUtil || _load_builderUtil()).log)("Building NSIS installer");
            try {
    ...
    buildInstaller() {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            packager.info.dispatchArtifactCreated({
                ...
                safeArtifactName: _this4.generateGitHubInstallerName(),
    ...
    executeMakensis(defines, commands, script) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            const nsisPath = yield NSIS_PATH.value;
