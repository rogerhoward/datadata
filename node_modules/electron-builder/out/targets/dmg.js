...
class DmgTarget extends (_core || _load_core()).Target {
    ...
    build(appPath, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            yield (0, (_fsExtraP || _load_fsExtraP()).outputFile)(_path.join(backgroundDir, "DSStorePlaceHolder"), Buffer.allocUnsafe(preallocatedSize));
            //noinspection SpellCheckingInspection
            yield (0, (_builderUtil || _load_builderUtil()).spawn)("hdiutil", addVerboseIfNeed(["create", "-srcfolder", backgroundDir, "-srcfolder", appPath, "-volname", volumeName, "-anyowners", "-nospotlight", "-fs", "HFS+", "-fsargs", "-c c=64,a=16,e=16", "-format", "UDRW"]).concat(tempDmg));
            ...
            const isContinue = yield (0, (_dmgUtil || _load_dmgUtil()).attachAndExecute)(tempDmg, true, (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
                const asyncTaskManager = new (_builderUtil || _load_builderUtil()).AsyncTaskManager(packager.info.cancellationToken);
                asyncTaskManager.addTask(specification.background == null ? (0, (_fsExtraP || _load_fsExtraP()).remove)(`${volumePath}/.background`) : (0, (_fsExtraP || _load_fsExtraP()).unlink)(`${volumePath}/.background/DSStorePlaceHolder`));
                ...
                for (const c of specification.contents) {
                    ...
                    if (c.type === "link") {
                        asyncTaskManager.addTask((0, (_builderUtil || _load_builderUtil()).exec)("ln", ["-s", `/${entryPath}`, `${volumePath}/${entryName}`]));
                    } else if (c.type === "file" && c.path != null) {
                        asyncTaskManager.addTask((0, (_fs || _load_fs()).copyFile)((yield packager.getResource(entryPath)), `${volumePath}/${entryName}`));
                    } else if (c.type === "dir" && c.path != null) {
                        asyncTaskManager.addTask((0, (_fs || _load_fs()).copyDir)((yield packager.getResource(entryPath)), `${volumePath}/${entryName}`));
                    }
                }
                yield (0, (_dmgUtil || _load_dmgUtil()).applyProperties)(entries, env, asyncTaskManager, packager);
